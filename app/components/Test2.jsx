  ...
  return (
    <div className="flex justify-between items-center h-16 w-full shadow-md px-4 md:px-16 relative">
      ...
      <div className="flex items-center" ref={searchRef}>
        {!isSearchOpen && (
          <form className="hidden md:flex relative flex-col">
            <div className="relative">
              <input 
                ...
                type="text"
                placeholder={` ${loadingData ? 'Patientez Kawou...' : 'Rechercher maintenant'}   `}
              />
              <button
                ...
                {searchResults.length > 0 ? (
                  searchResults.map((result, index) => (
                    <Link 
                      key={index} 
                      href={
                        result.type === 'Todo' ? `/todos/${result.item.id}` :
                        result.type === 'Etude' ? `/courses/${result.item.id}` :
                        `/vacances/${result.item.id}`
                      }
                    >
                      <div className="p-3 hover:bg-gray-100 cursor-pointer border-b border-gray-100">
                        <div className="font-medium">{result.item.title}</div>
