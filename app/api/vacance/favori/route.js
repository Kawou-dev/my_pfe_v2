...
//         const favoris = await Vacances.find({userId : session.user.id , favori : true  }).limit(3)
//         console.log("-------->", favoris) ; 
...
export async function GET() {
  try {
    ...
    const favoris = await Vacances.find({
      userId: session.user.id,
      favori: true,
    });

   
    const allImages = favoris.flatMap((vac) => vac.images).slice(0, 3);
